...
The active workout screen is the screen most likely to remain visible on a treadmill in landscape. At 844×390 and 844×320, the current 50/50 layout gives the live interval and the upcoming interval similar visual weight. When wake lock is unavailable, its guidance and the motion control create another competing block. The screen remains usable, but the live effort and countdown are not the obvious glance target.
...
- Weight the landscape grid approximately 65/35. The current phase, effort, countdown, incline, and cue dominate the left; the upcoming interval, End action, motion control, and wake-lock guidance form a compact right rail.
- Keep elapsed, progress, remaining, Up next, End, motion, and wake-lock guidance visible. Keep controls at least 48px and preserve safe-area insets.
...
| End and motion controls | Existing buttons |
...
- Elapsed, progress, remaining, upcoming interval, End, and motion remain visible, and interactive targets remain at least 48px.
