...
import { getResultHeadlineKey } from './resultHeadline'
...
describe('Given multiple workout sessions finish', () => {
  it('selects varied completion headlines that remain stable across reloads', () => {
    const headlines = Array.from({ length: 12 }, (_, index) => {
      ...
      expect(getResultHeadlineKey(plan, summary)).toBe(getResultHeadlineKey(plan, summary))
      return getResultHeadlineKey(plan, summary)
    })
  ...
  it('uses supportive rather than celebratory copy when a session ends early', () => {
    ...
    const completed = getResultHeadlineKey(plan, result('completed', '2026-09-21T05:00:00.000Z'))
    const ended = getResultHeadlineKey(plan, result('ended', '2026-09-21T05:00:00.000Z'))
